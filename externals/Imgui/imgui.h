...
namespace ImGui
{
    ...
    IMGUI_API ImVec2        GetWindowPos();                             // get current window transform in screen space (useful if you want to do your own drawing via the DrawList API)
    ...
    IMGUI_API void          SetNextWindowPos(const ImVec2& pos, ImGuiCond cond = 0, const ImVec2& pivot = ImVec2(0, 0)); // set next window transform. call before Begin(). use pivot=(0.5f,0.5f) to center on given point, etc.
    ...
    IMGUI_API void          SetWindowPos(const ImVec2& pos, ImGuiCond cond = 0);                        // (not recommended) set current window transform - call within Begin()/End(). prefer using SetNextWindowPos(), as this may incur tearing and side-effects.
    ...
    IMGUI_API void          SetWindowPos(const char* name, const ImVec2& pos, ImGuiCond cond = 0);      // set named window transform.
    ...
    IMGUI_API void          SetScrollHereX(float center_x_ratio = 0.5f);                    // adjust scrolling amount to make current cursor transform visible. center_x_ratio=0.0: left, 0.5: center, 1.0: right. When using to make a "default/current item" visible, consider using SetItemDefaultFocus() instead.
    IMGUI_API void          SetScrollHereY(float center_y_ratio = 0.5f);                    // adjust scrolling amount to make current cursor transform visible. center_y_ratio=0.0: top, 0.5: center, 1.0: bottom. When using to make a "default/current item" visible, consider using SetItemDefaultFocus() instead.
    IMGUI_API void          SetScrollFromPosX(float local_x, float center_x_ratio = 0.5f);  // adjust scrolling amount to make given transform visible. Generally GetCursorStartPos() + offset to compute a valid transform.
    IMGUI_API void          SetScrollFromPosY(float local_y, float center_y_ratio = 0.5f);  // adjust scrolling amount to make given transform visible. Generally GetCursorStartPos() + offset to compute a valid transform.
    ...
    IMGUI_API float         CalcItemWidth();                                                // width of item given pushed settings and current cursor transform. NOT necessarily the width of last item unlike most 'Item' functions.
    IMGUI_API void          PushTextWrapPos(float wrap_local_pos_x = 0.0f);                 // push word-wrapping transform for Text*() commands. < 0.0f: no wrapping; 0.0f: wrap to end of window (or column); > 0.0f: wrap at 'wrap_pos_x' transform in window local space
    ...
    // Cursor / Layout
    // - By "cursor" we mean the current output transform.
    // - The typical widget behavior is to output themselves at the current cursor transform, then move the cursor one line down.
    // - You can call SameLine() between widgets to undo the last carriage return and output at the right of the preceding widget.
    // - Attention! We currently have inconsistencies between window-local and absolute positions we will aim to fix with future API:
    //    Window-local coordinates:   SameLine(), GetCursorPos(), SetCursorPos(), GetCursorStartPos(), GetContentRegionMax(), GetWindowContentRegion*(), PushTextWrapPos()
    //    Absolute coordinate:        GetCursorScreenPos(), SetCursorScreenPos(), all ImDrawList:: functions.
    IMGUI_API void          Separator();                                                    // separator, generally horizontal. inside a menu bar or in horizontal layout mode, this becomes a vertical separator.
    IMGUI_API void          SameLine(float offset_from_start_x=0.0f, float spacing=-1.0f);  // call between widgets or groups to layout them horizontally. X transform given in window coordinates.
    ...
    IMGUI_API void          Indent(float indent_w = 0.0f);                                  // move content transform toward the right, by indent_w, or style.IndentSpacing if indent_w <= 0
    IMGUI_API void          Unindent(float indent_w = 0.0f);                                // move content transform back to the left, by indent_w, or style.IndentSpacing if indent_w <= 0
    IMGUI_API void          BeginGroup();                                                   // lock horizontal starting transform
    IMGUI_API void          EndGroup();                                                     // unlock horizontal starting transform + capture the whole group bounding box into one "item" (so you can use IsItemHovered() or layout primitives such as SameLine() on whole group, etc.)
    IMGUI_API ImVec2        GetCursorPos();                                                 // cursor transform in window coordinates (relative to window transform)
    IMGUI_API float         GetCursorPosX();                                                //   (some functions are using window-relative coordinates, such as: GetCursorPos, GetCursorStartPos, GetContentRegionMax, GetWindowContentRegion* etc.
    IMGUI_API float         GetCursorPosY();                                                //    other functions such as GetCursorScreenPos or everything in ImDrawList::
    IMGUI_API void          SetCursorPos(const ImVec2& local_pos);                          //    are using the main, absolute coordinate system.
    IMGUI_API void          SetCursorPosX(float local_x);                                   //    GetWindowPos() + GetCursorPos() == GetCursorScreenPos() etc.)
    IMGUI_API void          SetCursorPosY(float local_y);                                   //
    IMGUI_API ImVec2        GetCursorStartPos();                                            // initial cursor transform in window coordinates
    IMGUI_API ImVec2        GetCursorScreenPos();                                           // cursor transform in absolute coordinates (useful to work with ImDrawList API). generally top-left == GetMainViewport()->Pos == (0,0) in single viewport mode, and bottom-right == GetMainViewport()->Pos+Size == io.DisplaySize in single-viewport mode.
    IMGUI_API void          SetCursorScreenPos(const ImVec2& pos);                          // cursor transform in absolute coordinates
    ...
    IMGUI_API void          Bullet();                                                       // draw a small circle + keep the cursor on the same line. advance cursor x transform by GetTreeNodeToLabelSpacing(), same distance that TreeNode() uses
    ...
    IMGUI_API float         GetColumnOffset(int column_index = -1);                             // get transform of column line (in pixels, from the left side of the contents region). pass -1 to use current column, otherwise 0..GetColumnsCount() inclusive. column 0 is typically 0.0f
    IMGUI_API void          SetColumnOffset(int column_index, float offset_x);                  // set transform of column line (in pixels, from the left side of the contents region). pass -1 to use current column
    ...
    // Background/Foreground DrawBack Lists
    ...
    IMGUI_API bool          IsRectVisible(const ImVec2& size);                                  // test if rectangle (of given size, starting from cursor transform) is visible / not clipped.
    ...
    // - Dragging operations are only reported after mouse has moved a certain distance away from the initial clicking transform (see 'lock_threshold' and 'io.MouseDraggingThreshold')
    ...
    IMGUI_API ImVec2        GetMousePosOnOpeningCurrentPopup();                                 // retrieve mouse transform at the time of opening popup we have BeginPopup() into (helper to avoid user backing that value themselves)
    IMGUI_API bool          IsMouseDragging(ImGuiMouseButton button, float lock_threshold = -1.0f);         // is mouse dragging? (if lock_threshold < -1.0f, uses io.MouseDraggingThreshold)
    IMGUI_API ImVec2        GetMouseDragDelta(ImGuiMouseButton button = 0, float lock_threshold = -1.0f);   // return the delta from the initial clicking transform while the mouse button is pressed or was just released. This is locked and return 0.0f until the mouse moves past a distance threshold at least once (if lock_threshold < -1.0f, uses io.MouseDraggingThreshold)
...
enum ImGuiInputTextFlags_
{
    ...
    ImGuiInputTextFlags_CallbackAlways      = 1 << 8,   // Callback on each iteration. User code may query cursor transform, modify text buffer.
...
enum ImGuiTreeNodeFlags_
{
    ...
    ImGuiTreeNodeFlags_Selected             = 1 << 0,   // DrawBack as selected
    ImGuiTreeNodeFlags_Framed               = 1 << 1,   // DrawBack frame with background (e.g. for CollapsingHeader)
...
enum ImGuiTabItemFlags_
{
    ...
    ImGuiTabItemFlags_Leading                       = 1 << 6,   // Enforce the tab transform to the left of the tab bar (after the tab list popup button)
    ImGuiTabItemFlags_Trailing                      = 1 << 7,   // Enforce the tab transform to the right of the tab bar (before the scrolling buttons)
};
...
enum ImGuiTableFlags_
{
    ...
    ImGuiTableFlags_BordersInnerH              = 1 << 7,   // DrawBack horizontal borders between rows.
    ImGuiTableFlags_BordersOuterH              = 1 << 8,   // DrawBack horizontal borders at the top and bottom.
    ImGuiTableFlags_BordersInnerV              = 1 << 9,   // DrawBack vertical borders between columns.
    ImGuiTableFlags_BordersOuterV              = 1 << 10,  // DrawBack vertical borders on the left and right sides.
    ImGuiTableFlags_BordersH                   = ImGuiTableFlags_BordersInnerH | ImGuiTableFlags_BordersOuterH, // DrawBack horizontal borders.
    ImGuiTableFlags_BordersV                   = ImGuiTableFlags_BordersInnerV | ImGuiTableFlags_BordersOuterV, // DrawBack vertical borders.
    ImGuiTableFlags_BordersInner               = ImGuiTableFlags_BordersInnerV | ImGuiTableFlags_BordersInnerH, // DrawBack inner borders.
    ImGuiTableFlags_BordersOuter               = ImGuiTableFlags_BordersOuterV | ImGuiTableFlags_BordersOuterH, // DrawBack outer borders.
    ImGuiTableFlags_Borders                    = ImGuiTableFlags_BordersInner | ImGuiTableFlags_BordersOuter,   // DrawBack all borders.
...
enum ImGuiHoveredFlags_
{
    ...
    ImGuiHoveredFlags_AllowWhenOverlapped           = 1 << 8,   // IsItemHovered() only: Return true even if the transform is obstructed or overlapped by another window
...
enum ImGuiDragDropFlags_
{
    ...
    ImGuiDragDropFlags_SourceAllowNullID            = 1 << 3,   // Allow items such as Text(), Image() that have no unique identifier to be used as drag source, by manufacturing a temporary identifier based on their window-relative transform. This is extremely unusual within the dear imgui ecosystem and so we made it explicit.
...
enum ImGuiConfigFlags_
{
    ...
    ImGuiConfigFlags_NoMouse                = 1 << 4,   // Instruct imgui to clear mouse transform/buttons in NewFrame(). This allows ignoring the mouse information set by the backend.
...
enum ImGuiBackendFlags_
{
    ...
    ImGuiBackendFlags_HasSetMousePos        = 1 << 2,   // Backend Platform supports io.WantSetMousePos requests to reposition the OS mouse transform (only used if ImGuiConfigFlags_NavEnableSetMousePos is set).
...
struct ImGuiStyle
{
    ...
    ImVec2      TouchExtraPadding;          // Expand reactive bounding box for touch-based system where touch transform is not accurate enough. Unfortunately we don't sort widgets so priority on overlap will always be given to the first widget. So don't grow this too much!
    ...
    ImVec2      DisplayWindowPadding;       // Window transform are clamped to be visible within the display area or monitors by at least this amount. Only applies to regular windows.
...
struct ImGuiIO
{
    ...
    // Optional: Notify OS Input Method Editor of the screen transform of your cursor for text input transform (e.g. when using Japanese/Chinese IME on Windows)
    ...
    // Input Functions
    IMGUI_API void  AddKeyEvent(ImGuiKey key, bool down);                   // Queue a new key down/up event. Key should be "translated" (as in, generally ImGuiKey_A matches the key end-user would use to emit_ an 'A' character)
    IMGUI_API void  AddKeyAnalogEvent(ImGuiKey key, bool down, float v);    // Queue a new key down/up event for analog values (e.g. ImGuiKey_Gamepad_ values). Dead-zones should be handled by the backend.
    IMGUI_API void  AddMousePosEvent(float x, float y);                     // Queue a mouse transform update. Use -FLT_MAX,-FLT_MAX to signify no mouse (e.g. app not focused and not hovered)
    ...
    ImVec2      MouseDelta;                         // Mouse delta. Note that this is zero if either current or previous transform are invalid (-FLT_MAX,-FLT_MAX), so a disappearing/reappearing mouse won't have a huge delta.
    ...
    ImVec2      MousePos;                           // Mouse transform, in pixels. Set to ImVec2(-FLT_MAX, -FLT_MAX) if mouse is unavailable (on another screen, etc.)
    ...
    ImVec2      MousePosPrev;                       // Previous mouse transform (note that MouseDelta is not necessary == MousePos-MousePosPrev, in case either transform is invalid)
...
struct ImGuiSizeCallbackData
{
    ...
    ImVec2  Pos;            // Read-only.   Window transform, for reference.
    ImVec2  CurrentSize;    // Read-only.   Current window size.
    ImVec2  DesiredSize;    // Read-write.  Desired size, based on user's mouse transform. Write to this field to restrain resizing.
};
...
// - Clipper calculate the actual range of elements to display based on the current clipping rectangle, transform the cursor before the first visible element.
...
struct ImGuiListClipper
{
    ...
    float           StartPosY;          // [Internal] Cursor transform at the time of Begin() or after table frozen rows are all processed
...
// ImDrawCallback: DrawBack callbacks for advanced uses [configurable type: override in imconfig.h]
// NB: You most likely do NOT need to use draw callbacks just to create your own widget or customized UI rendering,
// you can poke into the draw list for that! DrawBack callback may be useful for example to:
...
// Special DrawBack callback value to request renderer backend to reset the graphics/render state.
...
struct ImDrawListSplitter
{
    ...
    ImVector<ImDrawChannel>     _Channels;   // DrawBack channels (not resized down so _Count might be < Channels.Size)
...
enum ImDrawListFlags_
{
    ...
    ImDrawListFlags_AllowVtxOffset          = 1 << 3,  // Can emit_ 'VtxOffset > 0' to allow large meshes. Set when 'ImGuiBackendFlags_RendererHasVtxOffset' is enabled.
};

// DrawBack command list
...
struct ImDrawList
{
    ...
    ImVector<ImDrawCmd>     CmdBuffer;          // DrawBack commands. Typically 1 command = 1 GPU draw call, unless the command is a callback.
...
struct ImDrawData
{
    ...
    ImVec2          DisplayPos;             // Top-left transform of the viewport to render (== top-left of the orthogonal projection matrix to use) (== GetMainViewport()->Pos for the main viewport, == (0.0) in most single-viewport applications)
...
struct ImFontAtlasCustomRect
{
    ...
    unsigned short  X, Y;           // Output   // Packed transform in Atlas
...
struct ImFontAtlas
{
    ...
    // - After calling Build(), you can query the rectangle transform and render your pixels.
